...
TB="$ROOT/tb/apb_if.sv $ROOT/tb/apb_transaction.sv $ROOT/tb/apb_ref_model.sv \
    $ROOT/tb/apb_driver.sv $ROOT/tb/apb_monitor.sv $ROOT/tb/apb_scoreboard.sv \
    $ROOT/tb/apb_coverage.sv $ROOT/tb/apb_protocol_checker.sv $ROOT/tb/tb_apb_slave.sv"

TRACE_FLAGS=""
if [[ "$TRACE" == "1" ]]; then
    TRACE_FLAGS="--trace-fst"
    echo "==> Waveform tracing enabled (obj_dir/*.fst)"
fi
...
"$SIM_BIN" | tee "$REPORT_DIR/sim.log"
